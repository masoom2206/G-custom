...
(function ($, Drupal, window, document, undefined) {
	...
	Drupal.behaviors.my_listing = {
		attach: function(context, settings) {
			jQuery(document).on('change','.pdf_brochure', function() { 
				if (this.checked) {
					//alert('checked');
				 }
				 else{
			...
			jQuery(document).on('click','.shared_listing_download', function() { 
			...
			jQuery(document).on('click','.share_listing', function() { 
			...
			jQuery(document).on('click','.listing-pdf #agent_submit #edit-submit', function() { 
			...
			/* Changes mady by Nitesh 12-09-2016 */
			jQuery(".my-listings-part-one").mouseenter(function() {
				jQuery(this).find('.mc-order-status').animate({right:'-100%'}, {duration: 700});
			});
			jQuery(".my-listings-part-one").mouseleave(function() {
				jQuery(this).find('.mc-order-status').animate({right:'2px'}, {duration: 700});
			});		
			...
			//code added by Harinder Singh Maan on 27-09-2016
			jQuery('#edit-field-aam-profile-override-dev-und-1').click(function(e){
				if (this.checked) {
...
jQuery('.design-thumb').click(function(){
    alert("Oops, this feature is only available for Marketing Concierge orders. If you’d like to super charge the marketing of your listing with an enhanced single property website, video, eBlast, property brochures, postcards and more, plus have your Marketing Coordinator do it all for you, upgrade to <a href='/marketing-concierge/[listingNID]'> Marketing Concierge</a> now");
});*/