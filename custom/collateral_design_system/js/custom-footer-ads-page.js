function addShirtImage() {
		fabric.Image.fromURL('http://dev.cbone.me/sites/default/files/cbone-banner-small.png', function(image) {
		  image.set({
			left: 90,
			top: 100,
			angle: (0, 0)
	...
	function addAccessors($scope) {
	  ...
	  $scope.addText = function() {
		...
		var textSample = new fabric.Text(text, {
		  left: getRandomInt(250, 300),
		  top: getRandomInt(250, 300),
	  ...
	  function addImage(imageName, minScale, maxScale) {
		...
		fabric.Image.fromURL('http://dev.cbone.me/sites/all/modules/custom/collateral_design_system/modules/property_ads/images/' + imageName, function(image) {
	 ...
	 $scope.addImage1 = function() {
	  ...
	  $scope.savePdf = function() {
		...
		var base_path =  "<?php echo $base_url.'/property-ads-ajax'; ?>";
		var pdf_file_path = "<?php echo $base_url.'/property-preview-ads'; ?>";
		
		//window.location.href = base_path + "?=mydata=" + canvasStringify;
		jQuery.ajax({
			type: "POST",
			dataType: "json",
			url: base_path,
			data: {mydata: imgData},
			success: window.open(pdf_file_path),
		});
		//var pdf = new jsPDF();
		//pdf.setFontSize(40);
		//pdf.text(35, 25, "CB|ONE");
		//pdf.addImage(fullQuality, 'JPEG', 15, 40, 180, 160);
	  ...
	  var pattern = new fabric.Pattern({
		source: 'http://dev.cbone.me/sites/all/modules/custom/collateral_design_system/modules/property_ads/images//logo.png',
		repeat: 'repeat'
	  });
