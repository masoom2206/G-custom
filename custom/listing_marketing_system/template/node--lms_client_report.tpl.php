<?php
  ...
  $sortitemsdata = array();
  foreach($node->field_client_report_items[LANGUAGE_NONE] as $key => $values) {
    $mkey = preg_replace('/[^A-Za-z0-9\-]/', '', $values['value']);
    $sortitemsdata[$mkey] = $values['value'];
  }

  foreach($node->field_completed_item2[LANGUAGE_NONE] as $key => $values) {
    $mkey = preg_replace('/[^A-Za-z0-9\-]/', '', $values['value']);
    $sortitemsdata[$mkey] = $values['value'];
  }
  $result = db_select('clinet_report_additinal_items', 'c')
      ->fields('c')
      ->condition('lnid', $listing_nid ,'=')
      ->execute()
      ->fetchAssoc();
  if (isset($result['items'])) {
    $order = explode("||", $result['items']);
  }
  $additionalitmes = drupal_json_decode($result['additional_items']);
  if (isset($additionalitmes)) {
    foreach ($additionalitmes as $key => $value) {
      $sortitemsdata[$key] = $value;
    }
  }
  ...
  if (isset($order) && count($order) > 0 ) {
    foreach( $order as $skey) {
      if (!empty($sortitemsdata[$skey]) ) {
       $clientreportitem .= '<li>' . $sortitemsdata[$skey].'</li>';
      }
    }
  } 
  else {
    foreach( $sortitemsdata as $key => $value) {
       $clientreportitem .= '<li>' . $value.'</li>';
    }
  }
  ...
  if (isset($listingpdf) && !empty($listingpdf)) {
    foreach ($listingpdf as $key => $value) {
       ...
       if ($listing_node->status == 1) {
         $nodelinkproperty = l('property website', 'node/'.$listing_node->nid);
         $nodelinkproperty = str_replace('www.homecb.com', $base_url, $nodelinkproperty);
         $clientreportitem = str_replace('property website', $nodelinkproperty, $clientreportitem);
       }
    }
  }
