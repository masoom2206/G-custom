...
(function ($, Drupal, window, document, undefined) {
	...
	Drupal.behaviors.listing_video = {
		attach: function(context, settings) {
			...
			//Added by Nitesh
			jQuery(document).on("dblclick","#sortable2 .slide-photos",function(e){
				if (jQuery(this).parent().attr("id") == "sortable2") {
					jQuery(this).remove().insertBefore(jQuery('.slide-second')).show('slow');
					jQuery('#sortable1 .slide-photos').removeAttr("style");
				}
			});
			jQuery(document).on("dblclick","#sortable1 .slide-photos",function(e){
				if (jQuery(this).parent().attr("id") == "sortable1") {
					jQuery(this).remove().appendTo("#sortable2").show('slow');
				}
			});
			...
			jQuery('#create-video-slide').click(function(e) {
				...
				jQuery.post('/listing-video/create', {"fids":fids, "nid":nid, "mp3":mp3, "zoom":zoom}, function(response) {
					if(response == 'done'){
						jQuery('div.ajax-progress-throbber').remove();
						location.reload();
						alert("The video has been created successfully!");
					}
			...
			var class_name = jQuery('#post_video_containers').attr( "class" );
			//alert(class_name);
			var class_name_array = class_name.split(' ');
			if(class_name_array[2] == 'active'){
				jQuery( "#create_video_container" ).css("display", "none");
				jQuery( "#post_video_container" ).css("display", "block");
			}
