...
namespace Drupal\autocomplete_advanced\Element;
...
/**
 * Provides an Autocomplete Advanced Form API element.
 *
 * @FormElement("autocomplete_advanced")
 */
class AutocompleteAdvancedElement extends FormElement {
  ...
  /**
   * Autocomplete Advanced element process callback.
   */
  public static function processElement($element) {
    $element['#attached']['library'][] = 'autocomplete_advanced/assets';

    // Workaround for problems with jquery css in seven theme.
    $active_theme = \Drupal::theme()->getActiveTheme();
    if ($active_theme->getName() == 'seven') {
      $element['#attached']['library'][] = 'autocomplete_advanced/assets.seven';
    }

    $html_id = Html::getUniqueId('autocomplete-advanced-input');
    ...
    $element['textfield'] = [
      ...
      '#attributes' => [
        'class' => ['autocomplete-advanced-form'],
        'id' => $html_id,
      ],
      '#default_value' => '',
      '#prefix' => '<div class="autocomplete-advanced-container">',
      '#suffix' => '</div>',
    ...
    if (isset($element['#autocomplete_advanced_path'])) {
      ...
      if ($element['#multiple']) {
        $element['value_field'] = [
          '#type' => 'textfield',
          '#attributes' => [
            'class' => ['autocomplete-advanced-value-field'],
          ],
          '#default_value' => $default_value,
          '#prefix' => '<div class="autocomplete-advanced-value-container">',
          '#suffix' => '</div>',
          '#description' => isset($element['#description']) ? $element['#description'] : '',
        ];
      ...
      $js_settings[$html_id] += [
        'type' => 'ajax',
        'uri' => $element['#autocomplete_advanced_path'],
      ];
    }
    else {
    ...
    $element['#attached']['drupalSettings']['autocomplete_advanced'] = $js_settings;
    $element['#tree'] = TRUE;
