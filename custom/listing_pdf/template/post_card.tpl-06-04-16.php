...
<div class="manage-listing-photos">
	<div class="listing-photos-header">
		<div class="photos-title">Post Cards</div>
	...
	foreach($template_details as $key=>$value) {
		...
		$output.='</div> <!--/brochure_image-- -->
			...
			<span class="edit_brochure">
				<a href="'.$base_url.'/generate-pdf/'.$var_name['listing_nid'].'/'.$node_load->nid.'">
				<img src="/sites/all/modules/custom/listing_marketing_system/images/pencil-edit.png"/></a>
			</span>';
			if($results !='' && !empty($results) && node_load($results)->uid==$user->uid) {
