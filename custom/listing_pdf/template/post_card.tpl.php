...
<div class="manage-listing-photos">
	<div class="listing-photos-header">
		<div class="photos-title">Postcards - Order <?php print $var_name['order_id']; ?></div>
	...
	foreach($template_id as $key=>$value) {
		...
		foreach($value as $nid) {
			...
			if($results !='' && !empty($results) && node_load($results)->uid==$user->uid) {
				//$output.='<div class="send-pdf"><a href=""><img src="/sites/all/modules/custom/listing_marketing_system/images/mail.png"/></a></div>';
			}
			...
			$postcard_type = isset($mcpdf_node->field_postcard_type['und']['0']['value']) ? $mcpdf_node->field_postcard_type['und']['0']['value'] : '';
			if($var_name['approve_listed'] && $var_name['approve_sold']) {
				if($var_name['approve_listed_pdf'] == $mcpdf_node->nid || $var_name['approve_sold_pdf'] == $mcpdf_node->nid) {
			...
			/* Added by nitesh */
			else if(($var_name['approve_listed_pdf'] == $mcpdf_node->nid && $var_name['approve_listed_pdf'] != 0) && ($var_name['approve_sold_pdf'] == 0 && $var_name['approve_sold_pdf'] == 0)) {
			// print "<pre>";print_r($var_name);exit;
					if($results !='' && !empty($results) && $access == 1) {
						...
						if( !empty($results) ){
							...
							if(isset($mcpdf_node->field_pdf_preview_image['und'])){
								foreach($mcpdf_node->field_pdf_preview_image['und'] as $value){
									...
									$output .= '<li><a href="'.$photo_url.'" rel="lightbox[gallery'.$x.']['.$mcpdf_node->title.']" alt="Zoom In" title="Zoom In"><img src="/sites/all/modules/custom/listing_marketing_system/images/magnify.png"/></a></li>';
								}	
							}
						}
						else {
			...
			//else if(($var_name['approve_listed'] && $postcard_type == 'listed') || ($var_name['approve_sold'] && $postcard_type == 'sold')) {
				else if(($var_name['approve_listed_pdf'] == $mcpdf_node->nid && $var_name['approve_listed_pdf'] != 0) || ($var_name['approve_sold_pdf'] == $mcpdf_node->nid && $var_name['approve_sold_pdf'] != 0)) {
			// print "<pre>";print_r($var_name);exit;
					if($results !='' && !empty($results) && $access == 1) {
						$output.='<span class="download_brochure"><a href="'.$base_url.'/download_pdf/'.$var_name['listing_nid'].'/'.$node_load->nid.'?destination='.$path.'"><img src="/sites/all/modules/custom/listing_marketing_system/images/download-arrow.png"/></a></span>';
					}
			...
			if(!empty($results)) {
				$postcard_type = isset($mcpdf_node->field_postcard_type['und']['0']['value']) ? $mcpdf_node->field_postcard_type['und']['0']['value'] : '';
				$re_use = isset($mcpdf_node->field_reuse_count['und']['0']['value']) ? $mcpdf_node->field_reuse_count['und']['0']['value'] : '0';
				if(isset($mcpdf_node->field_mc_pdf_status['und']['0']['tid'])) {
					...
					if($var_name['approve_listed'] && $var_name['approve_sold']) {
						if($var_name['approve_listed_pdf'] == $mcpdf_node->nid && $var_name['approve_sold_pdf'] == $mcpdf_node->nid) {
							$output .= '<div class="send-for-approval"><span>JUST LISTED APPROVED BY AGENT</span></div>';
							$output .= '<div class="send-for-approval"><span>JUST SOLD APPROVED BY AGENT</span></div>';
						}
						else if($var_name['approve_listed_pdf'] == $mcpdf_node->nid || $var_name['approve_sold_pdf'] == $mcpdf_node->nid) {
							$output .= '<div class="send-for-approval"><span>'.$mc_pdf_status.'</span></div>';
						}
					}
					/*** Added by nitesh *******/
					else if(($var_name['approve_listed'] && $postcard_type == 'listed')) {
						if($var_name['approve_listed_pdf'] == $mcpdf_node->nid) {
							$output .= '<div class="send-for-approval"><span>JUST LISTED APPROVED BY AGENT</span></div>';
							$output .= "<div class='reuse-pdf'><a href='".$base_url.'/generate-pdf/'.$var_name['listing_nid'].'/'.$node_load->nid."?reuse_as=sold' postcard-type='listed' class='reuse_approved_pdf'>RE-USE FOR JUST SOLD</a></div>";
						}
					}
					else if($postcard_type == 'sold' && $re_use == 1 && $mc_pdf_status == 'PDF Pending Agent Approval') {
						$output .= '<div class="send-for-approval"><span>JUST Listed APPROVED BY AGENT</span></div>';
						$output .= '<div class="send-for-approval"><span>Just sold submitted for approval</span></div>';
					}			
					else if(($var_name['approve_listed'] && $postcard_type == 'sold')) {
						if($var_name['approve_listed_pdf'] == $mcpdf_node->nid) {
							$output .= '<div class="send-for-approval"><span>JUST LISTED APPROVED BY AGENT</span></div>';
							$output .= '<div class="send-for-approval"><input type="checkbox" id="'.$results.'"/><input type="hidden" posttype="sold"/>Send for just sold approval</div>';
						}
						else if($mc_pdf_status == 'PDF Pending Agent Approval'){
							$output .= '<div class="send-for-approval"><span>Just sold submitted for approval</span></div>';
						}
					}
					else if(($var_name['approve_sold'] && $postcard_type == 'listed')) {
						$output .= '<div class="send-for-approval"><span>&nbsp;</span></div>';
					}
					/*********/
					else if($mc_pdf_status == 'PDF Pending Agent Approval') {
	...
	print $output;
	if(!empty($template_id) && (!$var_name['approve_listed'] || !$var_name['approve_sold'])) {
		print '<div class="submit-send-for-approval"><a href="#" id="submit-approval-for-'.$var_name['listing_nid'].'" class="submit-approval-link" listing-nid="'.$var_name['listing_nid'].'" pdf_section="post-card">Submit</a></div>';
	}
	?>
	</div>
</div>
