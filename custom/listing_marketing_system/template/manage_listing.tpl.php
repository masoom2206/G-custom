<?php
	...
	$office_ids = array();
	$office_ids[] = isset($user_detail->field_member_office['und']['0']['value']) ? $user_detail->field_member_office['und']['0']['value'] : '';
	if(isset($user_detail->og_user_node['und']['0']['target_id'])){
		foreach($user_detail->og_user_node['und'] as $value){
			$user_gid = $value['target_id'];
			$office_node = node_load($user_gid);
			if($office_node->type=='office_group'){
				$office_ids[] = $office_node->field_office_id['und']['0']['value'];
			}			
		}
	}
	...
	//office Admin roles
	$office_admin_roles = array("Siteadmin", "Administrator", "Office Administrator", "Manager", "Marketing Coordinator");
	$office_admin_roles_result = array_intersect($office_admin_roles, $user->roles);
	...
	$photography_link = ($var_name['nid'] != NULL) ? '/manage-listing/'.$var_name['nid'].'/photography' : '';
	...
	if($var_name['product'] != ''){
		if($var_name['product'] == 'Premier') {
			...
			$product = ' platinum_plus_product';
		}	
		else if($var_name['product'] == 'Prime') {
			$product_image_url = '/'.drupal_get_path('module', 'listing_marketing_system'). '/images/bell-icon-gray-sm.png';
	...
	if($var_name['marketing_coordinator_uid'] != ''){
		...
		if(!empty($office_admin_roles_result)){
			$term = taxonomy_term_load($var_name['mcc_order_status']);
			if($term->name != 'Order Complete' && $var_name['product'] == 'Prime') {
			$marketing_coordinator .= '<div class="accept-and-confirm-order">
					<div class="confirm-order">
						<span><a href="javascript:void(0);" listing-nid="'.$var_name['nid'].'" id="mcc-order-confirm" product="'.$var_name['product'].'">Mark Prime Order Completed</a></span>
					</div>
				</div>';
			}
		}
	}
	else if(in_array('Marketing Coordinator', $user->roles) && $var_name['mcc_order_status'] != 0 && in_array($var_name['listing_office_id'], $office_ids)){
		$term = taxonomy_term_load($var_name['mcc_order_status']);
		if($term->name == 'Pending MC Confirmation') {
		$marketing_coordinator = '<div class="accept-and-confirm-order">
				<div class="confirm-order">
					<span><a href="javascript:void(0);" listing-nid="'.$var_name['nid'].'" id="mcc-order-confirm" product="'.$var_name['product'].'">Accept and Confirm Order</a></span>
				</div>
			</div>';
		}
	}
...
<div class="manage-listing-tools">
	<div class="listing-tools-header">
		...
		<?php if($var_name['product'] != ''){ ?>
			<div class="purchase-product-logo<?php print $product;?>">
	...
	<?php if($var_name['order_status'] == 'incomplete' && !empty($mc_roles_result)){ ?>
	...
	<div class="manage-listing-tool-body">
		<div class="listing-tools-body">
			...
			<?php if(!empty($admin_roles_result)) { ?>
				...
				<ul>
					<li>
					  <a href="<?php print $photography_link; ?>" id="listing-photography">
