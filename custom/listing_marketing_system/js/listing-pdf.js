	...
	Drupal.behaviors.listing_pdf_photo = {
		attach: function(context, settings) {
			...
			// });
				...
				jQuery(document).on("dblclick","#sortable2 .photos-draggable",function(e){
					
					if (jQuery(this).parent().attr("id") == "sortable2") {
						jQuery(this).remove().appendTo("#sortable1").show('slow');
					}
					
				});
				jQuery(document).on("dblclick","#sortable1 .photos-draggable",function(e){
			...
			/*jQuery(".photos-draggable").dblclick(function(){
			 if (jQuery(this).parent().attr("id") != "sortable1") {
			  var img = jQuery(this).clone();
			  jQuery("#sortable1").append(img);
			  jQuery(this).remove();
			 }
			 else {
			  var img = jQuery(this).clone();
			  jQuery("#sortable2").append(img);
			  jQuery(this).remove();
			 }
			});*/
				...
				if(li_count < image_count){
					...
					jQuery.post('/listing-pdf/sortable', {"neworder":neworder, "mcpdf_nid":mcpdf_nid, "nid1":nid1, "nid2":nid2, "editproof":editproof}, function(response) {
						if(response == 'editproof1'){
							...
							var confirmation_msg = jQuery('.confirmation-msg-text').attr('value');
							//alert(confirmation_msg);
							jQuery('#image_popup .image_area').text(confirmation_msg);
