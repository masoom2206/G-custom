		...
		attach: function(context, settings) {
			...
			jQuery('.share_listing').live('click', function() { 
					...
					if(id.length > 0){
						...
						alert('Until there is a brochure created and the "Use for shared listing" radio button is checked, the listing can not be shared.');
						return false;
					}
			});
			...
			jQuery('#import-listing-photos').click(function(e){
				...
				jQuery.post('/feeds/photo-feed', {"nrt_id":nrt_id, "listing_nid":listing_nid, "photo_nid":photo_nid}, function(response) {
					if(response == 'done'){
						jQuery('div.ajax-progress-throbber').remove();
						alert("Photos has imported successfully!");
						location.reload();
					}
				});
			});
			...
			jQuery('.accept-and-confirm-order #mcc-order-confirm').click(function(e){
				...
				jQuery.post('/lms-listing/mcc-order-confirm', {"listing_nid":listing_nid}, function(response) {
					if(response == 'done'){
						jQuery('div.ajax-progress-throbber').remove();
						alert("Order has accepted successfully!");
						location.reload();
					}
				});
			});
